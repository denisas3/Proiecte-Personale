...
public class ChatSelectController {
    ...
    @FXML
    private void handleOpenChat() throws Exception {
        Utilizator selected = userTable.getSelectionModel().getSelectedItem();
        if (selected == null) return;

        openChatWindow(loggedUser, selected);
        openChatWindow(selected, loggedUser);

        ((Stage) userTable.getScene().getWindow()).close();
    }

    private void openChatWindow(Utilizator from, Utilizator to) throws Exception {
